...
public interface StatRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("""
            SELECT new ru.practicum.stats_dto.ViewStatsDto(h.app, h.uri, COUNT(h.ip) as hits )
            FROM EndpointHit h
    ...
    @Query("""
            SELECT new ru.practicum.stats_dto.ViewStatsDto(h.app, h.uri, COUNT(DISTINCT h.ip) as hits )
            FROM EndpointHit h
            WHERE h.timestamp BETWEEN :start AND :end
            AND (:uris IS NULL OR h.uri IN :uris)
            GROUP BY h.app, h.uri
